...
def gen_vector(book_name, contList, discList):
	...
	for row_index in range(1, sheet.nrows):  # skip header line
		...
		asite_1 = sheet.cell(row_index, 1).value
		asite_2 = sheet.cell(row_index, 2).value
		asite_3 = sheet.cell(row_index, 3).value
		bsite_1 = sheet.cell(row_index, 4).value
		bsite_2 = sheet.cell(row_index, 5).value
		bsite_3 = sheet.cell(row_index, 6).value
		xsite = sheet.cell(row_index, 7).value
		...
		formula_data.append([composition,asite_1,asite_2, asite_3, bsite_1, bsite_2, bsite_3, xsite, numberofelements, stability, formation_E])
	...
	formula_df = pd.DataFrame(formula_data, columns = ["composition", "asite_1", "asite_2", "asite_3", "bsite_1", "bsite_2", "bsite_3", "xsite", "numberofelements", "stability", "formation_E"])
	formula_df.to_csv("formula_data.csv")
